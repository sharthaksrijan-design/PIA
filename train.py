...
def ce_forward_and_grads(E_b, labels, W_enc, omega, W_cls, b_cls, W_hid, b_hid, K, lam_sharp=0.01):
    ...
    grad_sharp_phi = lam_sharp * 2 * np.sin(4 * np.pi * norm_phi)
    ...
    # Improved complex-valued gradient (Wirtinger)
...
def train(enc, train_embs, train_labels, val_embs, val_labels,
          N_INTENTS, K, D, epochs=600, batch_size=256, lam_sharp=0.005):
    ...
    print(f"Sequence-Optimized Training. {epochs} epochs. lam_sharp={lam_sharp}")
